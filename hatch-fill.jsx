...
function main() {
  ...
  function displayDialog() {
    ...
    var lineDegreeLabel = degreeGroup.add("statictext", undefined, 'Line Degree:');
    lineDegreeLabel.size = [125, 20];

    var lineDegreeText = degreeGroup.add("edittext", undefined, lineDegreeDefault, { readonly: false });
    lineDegreeText.size = [60, 25];
    ...
    ok.onClick = function () {
      try {
        ...
        lineDegree = parseFloat(lineDegreeText.text);
  ...
  function calculateTriangle(width, angle) {
    ...
    return { width: widthByDegree, height: heightByDegree };
  }
  ...
  function generateLinesByDegree(item, lines, degree, isCrossLines) {
    ...
    if (degree <= 45) {
      ...
      for (var i = 0; i <= numLines; i += 2) {
        lines.push(createLine(left, top - lineDistance * i, triangle.width, degree, isCrossLines))
      };
    } else {
      ...
      for (var i = 0; i <= numLines; i += 2) {
        lines.push(createLine(left + lineDistance * i, top, triangle.width, degree, isCrossLines))
      };
    }
  }
